%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Education}


%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

%---------------------------------------------------------
  \cventry
    {Ph.D. in Physics} % Degree
    {Chongqing University} % Institution
    {Chongqing, China}% Location
    {Sep 2022 - Jun 2028 (expected)} % Date(s)
    {
      \begin{cvitems} % Description(s) bullet points
        \item{Advisor: \href{https://inspirehep.net/authors/1456898?ui-citation-summary=true&ui-exclude-self-citations=true}{Prof. Jin Li}}
        \item{Research: GW modeling, moving-source and environmental effects, detector response, and parameter estimation}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {B.S. in Physics} % Degree
    {China West Normal University} % Institution
    {Nanchong, China}% Location
    {Sep 2018 - Jun 2022} % Date(s)
    {
      \begin{cvitems} % Description(s) bullet points
        \item{Advisors: \href{https://inspirehep.net/authors/1647692?ui-citation-summary=true&ui-exclude-self-citations=true}{Assoc. Prof. Di Wu} and \href{https://inspirehep.net/authors/1275221?ui-citation-summary=true&ui-exclude-self-citations=true}{Assoc. Prof. Guo-Ping Li}}
        \item{Research: Ground-based gravitational-wave detection and data analysis}
        \item{Thesis (in Chinese): An Analysis of the LIGO Gravitational Waves Data Based on Newtonian Approximate Model}
      \end{cvitems}
    }

%---------------------------------------------------------
\end{cventries}
